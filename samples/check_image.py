...
camera = cv2.VideoCapture('http://192.168.43.236:8080/?action=stream.html')
    ...
    if pre_frame is None:
        ...
        for c in contours:
            ...
            if cv2.contourArea(c) < 6000:
                ...
                #cv2.imwrite(save_path + str(time.strftime('%Y-%m-%d %H:%M:%S', time.localtime(time.time()))) + '.jpg',
                #            cropImg)
                # 申请缓存
                '''
                buffer_ = io.BytesIO()
                #cropImg.savefig(buffer_, format="png")
                buffer_.seek(0)
                # 转换为numpy array
                #ar = np.asarray(image)
